...
/*   Updated: 2021/03/26 17:59:09 by monoue           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		bring_largest_num_to_top(t_stacks *stacks, int stack_type)
{
	t_num	*stack;
...
void			deal_chunk_range(t_stacks *stacks, int chunk_max_nums[],
																size_t chunk_i)
{
	bool			first_try;
	...
	chunk_size = get_chunk_size(lstsize(stacks->stack_a)
													+ lstsize(stacks->stack_b));
	first_try = true;
	while (stacks->stack_a && lstsize(stacks->stack_a)
						&& (first_try || lstsize(stacks->stack_b) % chunk_size))
	{
		first_try = false;
		a_rotation_info = get_rotation_type_and_num_a(stacks->stack_a,
													chunk_max_nums, chunk_i);
		a_target_num = get_num_of_target_index(stacks->stack_a,
			get_nearest_target_index(stacks->stack_a, chunk_max_nums, chunk_i));
		b_rotation_info = get_rotation_type_and_num_b(stacks->stack_b,
																a_target_num);
		rotate_based_on_info(stacks, a_rotation_info, b_rotation_info);
